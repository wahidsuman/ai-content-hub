...
class AIWebsiteManager {
  ...
  async createArticle(newsItem, approved) {
    ...
    if (image && image.url) {
      if (image.source === 'dalle') {
        // DALL-E generated image with AI credit
        imageHtml = `
          <div class="article-image">
            <img src="${image.url}" alt="${image.alt}" style="width:100%;border-radius:12px;box-shadow:0 4px 6px rgba(0,0,0,0.1);">
            <p class="image-credit" style="font-size:12px;color:#666;margin-top:8px;text-align:right;font-style:italic;">
              🎨 ${image.credit || 'AI Generated by DALL-E 3 HD'}
            </p>
          </div>
        `;
  ...
  async getFreeImage(query) {
    try {
      ...
      console.log(`[DALL-E] Generating HD image for: ${query}`);
      
      // Call DALL-E 3 API with HD settings
      const response = await fetch('https://api.openai.com/v1/images/generations', {
        ...
        body: JSON.stringify({
          ...
          quality: 'hd',      // HD quality setting for best results
      ...
      if (response.ok) {
        ...
        if (data.data && data.data[0]) {
          console.log(`[DALL-E] ✅ Successfully generated HD image`);
          return {
            url: data.data[0].url,
            photographer: 'DALL-E 3 HD',
            photographerUrl: null,
            alt: query,
            source: 'dalle',
            type: 'generated',
            model: 'dall-e-3-hd',
            credit: 'AI Generated by DALL-E 3'
          };
        }
      } else {
